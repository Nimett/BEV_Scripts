...
def process_image(img_name, left_path, right_path, disp_save_dir, depth_save_dir, focal_length, baseline, stereo_matcher):
    ...
    disp_8bit = cv2.normalize(disparity, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX).astype(np.uint8)
    disp_orig_coloured = cv2.applyColorMap(disp_8bit, cv2.COLORMAP_JET)
    disp_orig_coloured[invalid_disp_mask] = [0, 0, 0]
    Image.fromarray(cv2.cvtColor(disp_orig_coloured, cv2.COLOR_BGR2RGB)).save(disp_save_dir/f"{img_name}_orig.png")

    # Save inpainted disparity map as npy and image file
    disparity_inpainted = cv2.inpaint(disp_8bit, invalid_disp_mask.astype(np.uint8) * 255, inpaintRadius=3, flags=cv2.INPAINT_TELEA)
    disp_inpainted_orig_scale = cv2.normalize(disparity_inpainted, None, alpha=(disparity[disparity > 0]).min(), beta=disparity.max(), norm_type=cv2.NORM_MINMAX)
    np.save(disp_save_dir/f"{img_name}_inpainted.npy", disp_inpainted_orig_scale)
    disparity_inpainted_colored = cv2.applyColorMap(np.uint8(disparity_inpainted), cv2.COLORMAP_JET)
    Image.fromarray(cv2.cvtColor(disparity_inpainted_colored, cv2.COLOR_BGR2RGB)).save(disp_save_dir / f"{img_name}.png")

    # Compute the depth map and save
    np.save(depth_save_dir / f"{img_name}_inpainted.npy", (focal_length * baseline) / disp_inpainted_orig_scale)


def main(left_img_dir, right_img_dir, output_dir, focal_length, baseline, num_jobs=8):
    ...
    for img_name, left_path in tqdm(left_imgs.items(), total=len(left_imgs)):
        ...
        process_image(img_name, left_path, right_imgs[img_name], disp_save_dir, depth_save_dir, focal_length, baseline, stereo_matcher)
...
if __name__ == "__main__":
    ...
    main(args.left_img_path, args.right_img_path, args.output_dir, args.focal_length, args.baseline)